...
var ui = {
  renderPosts: function renderPosts(posts) {
    var elements = posts.map(function (post) {
      ...
      var last_reply = post.last_reply;
  ...
  renderUsers: function renderUsers(activeUsers) {
    var elements = activeUsers.map(function (user) {
      // ES2015 Object destructuring
      var name = user.name;
...
function articleTemplate(title, last_reply) {
  ...
  var safeReply = _xssFilters2.default.inHTMLData(last_reply);
